...
import { MapPin, Calendar, FileText, ArrowLeft, Briefcase, Mail, Phone, Hash, Cake, UserCheck } from "lucide-react";
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3 mb-8">
          <Card className="border-l-4 border-l-indigo-500 shadow-sm hover:shadow-md transition-shadow">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium text-gray-600">Scadenze Totali</CardTitle>
              <FileText className="h-5 w-5 text-indigo-500" />
            </CardHeader>
            <CardContent>
              <div className="text-3xl font-bold text-indigo-600">{person.deadlineInstances.length}</div>
            </CardContent>
          </Card>

          <Card className="border-l-4 border-l-green-500 shadow-sm hover:shadow-md transition-shadow">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium text-gray-600">Ruoli Assegnati</CardTitle>
              <Briefcase className="h-5 w-5 text-green-500" />
            </CardHeader>
            <CardContent>
              <div className="text-3xl font-bold text-green-600">{person.roleAssignments.length}</div>
            </CardContent>
          </Card>
          ...
          <Card className="border-l-4 border-l-purple-500 shadow-sm hover:shadow-md transition-shadow">
            ...
            <CardContent>
              <div className="text-3xl font-bold text-purple-600">{person.structures.length}</div>
            </CardContent>
          </Card>
        </div>
            ...
            <CardContent className="space-y-4">
              ...
              {person.fiscalCode && (
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-gray-800 font-mono">{person.fiscalCode}</p>
                  </div>
                </div>
              )}
              ...
              {person.birthDate && (
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-gray-600">Data di Nascita</p>
              ...
              {person.hireDate && (
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-gray-600">Data Assunzione</p>
                    <p className="text-gray-800">{formatDate(person.hireDate)}</p>
                  </div>
                </div>
              )}
          ...
          <Card className="shadow-md border-t-4 border-t-purple-500">
            ...
            <CardContent className="space-y-4">
              {person.roleAssignments.length > 0 && (
                <div>
                  <p className="text-sm font-medium text-gray-600 mb-2">Ruoli Assegnati</p>
              ...
              {person.structures.length > 0 && (
                <div>
                  <p className="text-sm font-medium text-gray-600 mb-2">Strutture</p>
                  <div className="space-y-2">
                    {person.structures.map((ps) => (
                      ...
                      >
                        <p className="font-semibold text-gray-800">{ps.structure.name}</p>
              ...
              {person.roleAssignments.length === 0 && person.structures.length === 0 && (
                  ...
                  >
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <p className="font-semibold text-gray-800">{instance.title}</p>
                        <div className="flex items-center gap-4 mt-2 text-xs text-gray-500">
                          ...
                          <span className={`px-2 py-1 rounded-full font-medium ${
                            instance.status === "DONE" ? "bg-green-100 text-green-700" :
                            instance.status === "OVERDUE" ? "bg-red-100 text-red-700" :
                            instance.status === "CANCELLED" ? "bg-gray-100 text-gray-700" :
